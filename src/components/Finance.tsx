...
export default function Finance() {
  ...
  function addEntry() {
    ...
    const saved =
      (parseFloat(newEntry.income) || 0) -
      (parseFloat(newEntry.spent) || 0) -
      (parseFloat(newEntry.invested) || 0)
    setMonthlyLog((l) => [
      ...l,
      {
        ...newEntry,
    ...
    <div className="fade">
      ...
      >
        {[
          {
            label: 'Projected NW',
            pct: Math.min(Math.round((projectedNetWorth / 148000) * 100), 100),
            color: '#22c55e',
            sub: '$' + Math.round(projectedNetWorth / 1000) + 'k / $148k',
          },
          {
            label: 'Invested Total',
            pct: Math.min(Math.round((totalInvested / 24000) * 100), 100),
            color: '#f59e0b',
            sub: '$' + totalInvested.toLocaleString() + ' / $24k yr',
          },
          {
            label: 'Savings Rate',
            pct: savingsRate,
          ...
          >
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#e8e6e1' }}>{c.label}</div>
        ...
        {BUDGET.map((b, i) => {
          ...
          return (
            <div key={i} style={{ marginBottom: 10 }}>
              ...
              <div className="bar">
                <div className="fill" style={{ width: pct + '%', background: b.color }}></div>
              </div>
            </div>
          )
        })}
      ...
      <div className="card" style={{ padding: 13, marginBottom: 12 }}>
        ...
        {SAVINGS_DEPLOYMENT.map((r, i) => (
          <div key={i} style={{ marginBottom: 12 }}>
            ...
            <div className="bar">
              <div className="fill" style={{ width: r.pct + '%', background: r.color }}></div>
            </div>
          </div>
        ))}
      </div>
        ...
        {monthlyLog.map((m, i) => (
          ...
          >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
              {[
                { l: 'Income', v: '$' + (m.income || 0).toLocaleString(), c: '#22c55e' },
                { l: 'Spent', v: '$' + (m.spent || 0).toLocaleString(), c: '#ef4444' },
                { l: 'Invested', v: '$' + (m.invested || 0).toLocaleString(), c: '#f59e0b' },
                { l: 'Saved', v: '$' + (m.saved || 0).toLocaleString(), c: '#818cf8' },
              ].map((s, j) => (
                <div key={j}>
                  <div style={{ fontSize: 8, color: '#444', letterSpacing: 1 }}>{s.l}</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: s.c, marginTop: 2 }}>
                    {s.v}
                  </div>
                </div>
              ))}
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            {[
              ...
              <div key={f.k} style={{ marginBottom: 10 }}>
                ...
                <input
                  value={newEntry[f.k]}
                  onChange={(e) => setNewEntry((x) => ({ ...x, [f.k]: e.target.value }))}
                  placeholder={f.p}
                />
              </div>
            ))}
